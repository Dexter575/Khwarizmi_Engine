...
{
    using variant_type = const variant_lib::variant<int, double, std::string>;
    ...
    REQUIRE(variant_lib::apply_visitor(visitor, var1) == 124);
    REQUIRE(variant_lib::apply_visitor(visitor, var2) == 4);
    REQUIRE(variant_lib::apply_visitor(visitor, var3) == 0);
}
...
TEST_CASE("rvalue visitor works on const variants", "[visitor][unary visitor]")
{
    ...
    REQUIRE(variant_lib::apply_visitor(some_visitor{1}, var1) == 124);
    REQUIRE(variant_lib::apply_visitor(some_visitor{1}, var2) == 4);
    REQUIRE(variant_lib::apply_visitor(some_visitor{1}, var3) == 0);
}
...
TEST_CASE("visitor works on rvalue variants", "[visitor][unary visitor]")
{
    ...
    REQUIRE(variant_lib::apply_visitor(some_visitor{1}, variant_type{123}) == 124);
    REQUIRE(variant_lib::apply_visitor(some_visitor{1}, variant_type{3.2}) == 4);
    REQUIRE(variant_lib::apply_visitor(some_visitor{1}, variant_type{"foo"}) == 0);
}
...
{
    using variant_type = variant_lib::variant<int, std::string, double>;
    variant_type v;
    total_sizeof ts;
    v = 5.9;
    REQUIRE(variant_lib::apply_visitor(ts, v) == sizeof(double));
...
{
    using variant_type = const variant_lib::variant<int, std::string, double>;
    variant_type v{"foo"};
    const total_sizeof ts;
    REQUIRE(variant_lib::apply_visitor(ts, v) == sizeof(std::string));
