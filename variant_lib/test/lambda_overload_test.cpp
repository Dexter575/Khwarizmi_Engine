...
#include <variant_lib/variant_visitor.hpp>

#if __cplusplus >= 201402L
#define HAS_CPP14_SUPPORT
#endif

using namespace variant_lib;
...
using Either = variant_lib::variant<Left, Right>;
...
#ifdef HAS_CPP14_SUPPORT
{
    ...
    using Properties = variant_lib::variant<Center, Indent, Justify, None>;
...
void test_match_move_out_of_variant()
{
    ...
    using T3 = variant_lib::recursive_wrapper<int>;

    variant_lib::variant<T1, T2> v = T1{};

    std::move(v).match([](T1&&) {},  // Consume T1 by value
                       [](T2&&) {}); // Consume T2 by value

    variant_lib::variant<T3, T2> w = T2{};
