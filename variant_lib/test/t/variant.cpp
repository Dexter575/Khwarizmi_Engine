...
{
    using variant_type = variant_lib::variant<bool, std::string>;
...
TEST_CASE("variant should support built-in types", "[variant]")
{
    SECTION("bool")
    {
        variant_lib::variant<bool> v(true);
        REQUIRE(v.valid());
        REQUIRE(v.is<bool>());
        REQUIRE(v.which() == 0);
        ...
        v = true;
        REQUIRE(v == variant_lib::variant<bool>(true));
    }
    SECTION("nullptr")
    {
        using value_type = std::nullptr_t;
        variant_lib::variant<value_type> v(nullptr);
        REQUIRE(v.valid());
        REQUIRE(v.is<value_type>());
        REQUIRE(v.which() == 0);
        REQUIRE(v.get<value_type>() == nullptr);
        REQUIRE(v == variant_lib::variant<value_type>(nullptr));
    }
    SECTION("unique_ptr")
    {
        using value_type = std::unique_ptr<std::string>;
        variant_lib::variant<value_type> v(value_type(new std::string("hello")));
        REQUIRE(v.valid());
        REQUIRE(v.is<value_type>());
        REQUIRE(v.which() == 0);
    ...
    SECTION("string")
    {
        ...
        variant_lib::variant<value_type> v(value_type("hello"));
        REQUIRE(v.valid());
        REQUIRE(v.is<value_type>());
        REQUIRE(v.which() == 0);
        ...
        REQUIRE(v == variant_lib::variant<value_type>(value_type("variant")));
    }
    SECTION("size_t")
    {
        using value_type = std::size_t;
        variant_lib::variant<value_type> v(std::numeric_limits<value_type>::max());
        REQUIRE(v.valid());
        REQUIRE(v.is<value_type>());
        REQUIRE(v.which() == 0);
        ...
        v = value_type(1);
        REQUIRE(v == variant_lib::variant<value_type>(value_type(1)));
    }
    SECTION("int8_t")
    {
...
TEST_CASE("variant should support custom types", "[variant]")
{
    ...
    variant_lib::variant<MissionInteger> v(MissionInteger(34838300));
    REQUIRE(v.valid());
    ...
    v = MissionInteger(1);
    REQUIRE(v == variant_lib::variant<MissionInteger>(MissionInteger(1)));
}

TEST_CASE("variant::which() returns zero based index of stored type", "[variant]")
{
    using variant_type = variant_lib::variant<bool, std::string, std::uint64_t, std::int64_t, double, float>;
...
{
    using variant_type = variant_lib::variant<int, double>;
    variant_type var = 5;
    REQUIRE(var.is<int>());
    REQUIRE_FALSE(var.is<double>());
    REQUIRE(var.get<int>() == 5);
    REQUIRE_THROWS_AS(var.get<double>(),
                      variant_lib::bad_variant_access&);
}
...
TEST_CASE("get with wrong type (here: int) should throw", "[variant]")
{
    ...
    REQUIRE(variant_lib::get<double>(var) == 5.0);
    REQUIRE_THROWS_AS(var.get<int>(),
                      variant_lib::bad_variant_access&);
    REQUIRE_THROWS_AS(variant_lib::get<int>(var),
                      variant_lib::bad_variant_access&);
}
...
{
    variant_lib::variant<int, double> var{variant_lib::no_init()};
    REQUIRE_THROWS_AS(var.get<int>(),
                      variant_lib::bad_variant_access&);
    REQUIRE_THROWS_AS(var.get<double>(),
                      variant_lib::bad_variant_access&);
...
TEST_CASE("no_init variant can be copied and moved from", "[variant]")
{
    ...
    variant_type v1{variant_lib::no_init()};
    variant_type v2{42};
    variant_type v3{23};
...
TEST_CASE("no_init variant can be copied and moved to", "[variant]")
{
    ...
    variant_type v1{42};
    variant_type v2{variant_lib::no_init()};
    variant_type v3{variant_lib::no_init()};
...
{
    using variant_type = variant_lib::variant<int>;
...
{
    using variant_type = variant_lib::variant<long, char>;
...
{
    using variant_type = variant_lib::variant<unsigned char>;
...
{
    namespace detail = variant_lib::detail;
...
TEST_CASE("Type indexing should work with variants with duplicated types", "[variant::detail]")
{
    ...
    REQUIRE((variant_lib::detail::value_traits<bool, bool, int, double, std::string>::index == 3));
    REQUIRE((variant_lib::detail::value_traits<bool, bool, int, double, int>::index == 3));
    REQUIRE((variant_lib::detail::value_traits<int, bool, int, double, std::string>::index == 2));
    REQUIRE((variant_lib::detail::value_traits<int, bool, int, double, int>::index == 2));
    REQUIRE((variant_lib::detail::value_traits<double, bool, int, double, std::string>::index == 1));
    REQUIRE((variant_lib::detail::value_traits<bool, bool, int, bool, std::string>::index == 3));
    REQUIRE((variant_lib::detail::value_traits<std::string, bool, int, double, std::string>::index == 0));
    REQUIRE((variant_lib::detail::value_traits<dummy, bool, int, double, std::string>::index == variant_lib::detail::invalid_value));
    REQUIRE((variant_lib::detail::value_traits<std::vector<int>, bool, int, double, std::string>::index == variant_lib::detail::invalid_value));
}
...
TEST_CASE("variant default constructor", "[variant][default constructor]")
{
    ...
    REQUIRE_FALSE(variant_type{variant_lib::no_init()}.valid());
}
...
{
    using variant_type = variant_lib::variant<int, std::string>;
...
{
    using variant_type = variant_lib::variant<std::reference_wrapper<int>, std::reference_wrapper<double>>;

    int a = 1;
    variant_type v{std::ref(a)};
    REQUIRE(v.get<int>() == 1);
    REQUIRE(variant_lib::get<int>(v) == 1);
    REQUIRE_THROWS_AS(v.get<double>(),
                      variant_lib::bad_variant_access&);
    REQUIRE_THROWS_AS(variant_lib::get<double>(v),
                      variant_lib::bad_variant_access&);
    a = 2;
    REQUIRE(v.get<int>() == 2);
    v.get<int>() = 3;
    ...
    REQUIRE(variant_lib::get<double>(v) == Approx(3.141));
    REQUIRE_THROWS_AS(v.get<int>(),
                      variant_lib::bad_variant_access&);
    REQUIRE_THROWS_AS(variant_lib::get<int>(v),
                      variant_lib::bad_variant_access&);
    b = 2.718;
...
{
    using variant_type = variant_lib::variant<std::reference_wrapper<int const>, std::reference_wrapper<double const>>;

    int a = 1;
    variant_type v{std::cref(a)};
    REQUIRE(v.get<int const>() == 1);
    REQUIRE(v.get<int>() == 1);
    REQUIRE(variant_lib::get<int const>(v) == 1);
    REQUIRE(variant_lib::get<int>(v) == 1);
    REQUIRE_THROWS_AS(v.get<double const>(),
                      variant_lib::bad_variant_access&);
    REQUIRE_THROWS_AS(variant_lib::get<double const>(v),
                      variant_lib::bad_variant_access&);

    double b = 3.141;
    v = std::cref(b);
    REQUIRE(v.get<double const>() == Approx(3.141));
    REQUIRE(variant_lib::get<double const>(v) == Approx(3.141));
    REQUIRE_THROWS_AS(v.get<int const>(),
                      variant_lib::bad_variant_access&);
    REQUIRE_THROWS_AS(variant_lib::get<int const>(v),
                      variant_lib::bad_variant_access&);
}

TEST_CASE("recursive wrapper")
{
    using variant_type = variant_lib::variant<variant_lib::recursive_wrapper<int>>;
    variant_type v(1);
    REQUIRE(v.is<int>());
    REQUIRE(v.get<int>() == 1);
}
...
{
    using value = variant_lib::variant<bool, std::uint64_t>;
