#ifndef KE_UTIL_VARIANT_HPP
#define KE_UTIL_VARIANT_HPP
...
# ifdef __GNUC__
#  define KE_VARIANT_DEPRECATED __attribute__((deprecated))
# elif defined(_MSC_VER)
#  define KE_VARIANT_DEPRECATED __declspec(deprecated)
# else
#  define KE_VARIANT_DEPRECATED
# endif
#else
#  define KE_VARIANT_DEPRECATED [[deprecated]]
#endif
...
#if !defined(KE_VARIANT_MINIMIZE_SIZE)
using type_index_t = unsigned int;
#else
#if defined(KE_VARIANT_OPTIMIZE_FOR_SPEED)
...
public:
    ...
    // Use which() instead.
    KE_VARIANT_DEPRECATED VARIANT_INLINE type_index_t get_type_index() const
    {
        return type_index;
    }
    ...
    auto VARIANT_INLINE match(Fs&&... fs) const&
        -> decltype(variant::visit(*this, ::variant_lib::make_visitor(std::forward<Fs>(fs)...)))
    {
        return variant::visit(*this, ::variant_lib::make_visitor(std::forward<Fs>(fs)...));
    }
    // non-const
    template <typename... Fs>
    ...
    template <typename... Fs>
    auto VARIANT_INLINE match(Fs&&... fs) &&
        -> decltype(variant::visit(std::move(*this), ::variant_lib::make_visitor(std::forward<Fs>(fs)...)))
    {
        return variant::visit(std::move(*this), ::variant_lib::make_visitor(std::forward<Fs>(fs)...));
    }
...
namespace std {
template <typename... Types>
struct hash< ::variant_lib::variant<Types...>> {
    std::size_t operator()(const ::variant_lib::variant<Types...>& v) const noexcept
    {
        return ::variant_lib::apply_visitor(::variant_lib::detail::hasher{}, v);
    }
};

}; // namespace std

#endif // KE_UTIL_VARIANT_HPP
